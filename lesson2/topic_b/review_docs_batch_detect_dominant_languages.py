...
for name in os.listdir(".\pos_reduced"):
...
for file in textfile_list:
	path_and_file = cwd + "\\pos_reduced\\" + file
	f = open(path_and_file, "r", encoding='utf8')
	file = f.read()
	print(json.dumps(comprehend.detect_dominant_language(Text = file), sort_keys=True, indent=4))
